...
for a in range(frequency):
	point_list.append([coords_list[0][a],coords_list[1][a],coords_list[2][a],coords_list[3][a],coords_list[4][a]])

for a in point_list:
	for b in point_list:
		if point_list.index(a)<=point_list.index(b):
			if percolation_checker(a,b,r):
				if b[4] > a[4]:
					b[4] = a[4]
				else:
					a[4] = b[4]

b = sorted([a[4] for a in point_list])
prev = frequency
max_count = [0,0]
for item in b:
	if item == prev:
...
limit = scale_limit[1]
ax.scatter3D(coords_list[0],coords_list[1],coords_list[2],s=(80/9)*(r**2),c=[a[3] for a in point_list]) #I can create an array of the colours to represent percolation.
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
